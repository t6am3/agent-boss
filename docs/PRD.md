...
Boss 自己也需要 brain model 配置。

- 未配置时，Boss 进入 `rule fallback`：可以接目标、建 Mission、汇报进度、审计事件，但自然语言理解和调度判断较弱。
- 配置后，Boss 先用自己的 brain model 理解 Owner 意图，再调用 workspace / bash / browser / worker agent 推进任务。
- Boss brain 和 worker agent 是两层：Boss brain 负责管理判断，worker agent 负责具体执行。
...
| 2 | Boss Brain Config | 可为 Boss 自己配置 codex/claude/hermes 等 brain model；未配置时 rule fallback |
| 3 | Mission 创建 | 用户输入目标，Boss 创建可追踪任务 |
| 4 | Mission Progress | 默认只展示目标、进度、阻塞、风险、下一步 |
| 5 | 基础执行能力 | Boss 有自己的 mission workspace、文件系统、bash，必要时看浏览器 |
...
Boss brain configured: provider=codex, model=gpt-5.4
...
状态：执行完成
是否需要你：不需要
摘要：我已完成本地检查：project files inspected and npm test passed.
...
owner> 审计 m-001
Boss: 展开底层事件流、workspace 文件、bash 输出、子 agent 输出摘要、资源使用和代决策。
```

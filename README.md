...
Owner 提目标
    ↓
Agent Boss 管过程：workspace、文件系统、bash、浏览器观察、派发、确认、验收、汇报
    ↓
...
默认体验很窄：Owner 只给目标、只看进度。子 agent、runner、模型、命令、stdout/stderr 都是 Boss 的内部细节，只有在你说“审计”时才展开。

## 核心概念

- **Mission**：一个需要被管理到结束的目标，不只是一次 query。
- **Boss Direct Line**：自然语言交互式终端，Owner 只跟 Boss 单线沟通。
- **Boss Workspace**：Boss 自己的工作区，基础能力是文件系统、bash，必要时观察浏览器。
- **Asset Ledger**：内部台账，用于调度和审计，不是 Owner 日常要操作的工具面板。
...
- `mission run --runner workspace` 已接入 Boss 自己的 workspace/bash 执行循环，会为 mission 建工作区并记录 bash 审计。
- Mission ID 已改为 SQLite counter 事务领取，支持多个本地 Boss/runner 并发创建任务。
- `demo` 可以一键跑完整 MVP；`interactive` 可以进入交互式 shell。
- `boss` / `interactive` 已支持自然语言单线对话：直接说目标、问进度、要汇报、看审计、跑演示。
...
node dist/cli.js mission run m-001 --runner workspace
...
Boss workspace 保存在 `.agent-boss/workspaces/<missionId>/`，包含 `goal.md`、`plan.md`、
`progress.md` 和 `artifacts/` 下的 bash 输出。Owner 默认不看这些细节，只有说“审计”时展开。
...
2. 给 Boss workspace 补浏览器观察能力，形成文件系统 / bash / browser 三件套。
3. 增强 Boss 调度策略：workspace 自查不足时，Boss 自己按场景、成本、历史质量选择 worker/model。

...
- 当前 CLI 已支持 `assets add/update/list/show`、`mission create/status/watch/log/update/report/event/decide/complete`、`judge` 的本地监工闭环。
...
1. 继续补 AssetLedger / MissionStore / Supervisor / Reporter 的边界测试。
2. 设计 `MissionRunner` 接口，让 adapter 自动写入 progress / blocked / completed events。
3. 按新架构重写 Codex / Claude / OpenClaw adapter。
